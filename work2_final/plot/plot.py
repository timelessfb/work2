...
def plot_fun_slot(cost, fails, req_num_deta_eachtime, xlabel, ylabel, title):
    ...
    left, bottom, width, height = 0.1, 0.1, 0.8, 0.8
    ax1 = fig.add_axes([left, bottom, width, height])
    # ax11 = fig1.add_axes([left, bottom, width, height])

    # print("GASM")
    l1, = ax1.plot(x, y1, color='r', marker='o', markerfacecolor='w', linestyle='-')
    # l11, = ax11.plot(x, y1 * e1, color='r', marker='o', markerfacecolor='w', linestyle='-')
    # print("MBCSM")
    l2, = ax1.plot(x, y2, color='g', marker='1', markerfacecolor='w', linestyle='-')
    # l22, = ax11.plot(x, y2 * e2, color='g', marker='1', markerfacecolor='w', linestyle='-')
    # print("MCCSM")
    l3, = ax1.plot(x, y3, color='b', marker='^', markerfacecolor='w', linestyle='-')
    # l33, = ax11.plot(x, y3 * e3, color='b', marker='^', markerfacecolor='w', linestyle='-')
    # print("RSM")
    l4, = ax1.plot(x, y4, color='k', marker='D', markerfacecolor='w', linestyle='-')
    # l44, = ax11.plot(x, y4 * e4, color='k', marker='D', markerfacecolor='w', linestyle='-')
    # l5, = ax1.plot(x, y5, color='y', marker='o', markerfacecolor='w', linestyle='--')
    ax1.set_xlabel(xlabel, fontproperties=zhfont)
    # ax11.set_xlabel(xlabel, fontproperties=zhfont)
    print("切片请求个数")
    print(x)
    print("映射失败个数")
    print("GASM")
    ...
    print(x - e4)
    if ylabel == 1:
        ylabel = r'$\mathit{\overline{\phi}_c}$'
        ylabel1 = r'$\mathit{\phi_c}$'
        print("单位/总计算代价结果")
        print("GASM")
        print(y1)
        print(y1 * e1)
    ...
    ax1.set_ylabel(ylabel, fontproperties=zhfont)
    # ax11.set_ylabel(ylabel1, fontproperties=zhfont)
    # ax1.set_xticks(x)
    title1 = title
    title += '单位'
    title1 += '总'
    ...
    ax11.set_ylabel(ylabel1, fontproperties=zhfont)
    ax11.set_xlabel(xlabel, fontproperties=zhfont)
    plt.legend(handles=[l11, l22, l33, l44], labels=['GASM', 'MBCSM', 'MCCSM', 'RSM'], loc='best', prop=enfont)
